...
import { Card, CardHeader, CardBody, Button, Input, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, Divider, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, useDisclosure } from "@heroui/react";
import { useNavigate, useLocation } from "react-router-dom";
import { GetLanguageNames, GetBaseRoot, SetBaseRoot, ResetBaseRoot, GetInstallerDir, GetVersionsDir, CanWriteToDir } from "../../bindings/github.com/liteldev/LeviLauncher/minecraft";
...
export default function OnboardingPage() {
  ...
  const [langNames, setLangNames] = React.useState<Array<{ language: string; code: string }>>([]);
    ...
    (async () => {
      try {
        if (hasBackend) {
          ...
          setNewBaseRoot(typeof pick === "string" && pick.length > 0 ? String(pick) : String(br || ""));
  ...
  const proceedHome = () => {
    try { localStorage.setItem("ll.onboarded", "1"); } catch {}
    ...
    <div className="px-3 sm:px-5 lg:px-8 py-3 sm:py-4 lg:py-6 w-full max-w-none">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}>
        <Card className="rounded-3xl shadow-xl p-2 bg-white/60 dark:bg-black/30 backdrop-blur-md border border-white/30">
          <CardHeader className="flex flex-col items-start px-4 pb-0 pt-4">
            <div className="text-large font-semibold">{t("onboarding.title", { defaultValue: "首次启动引导" })}</div>
            <p className="text-small text-default-500">{t("onboarding.subtitle", { defaultValue: "请先设置内容路径与语言" })}</p>
          </CardHeader>
          <Divider className="my-2 bg-default-200/60 h-px" />
          <CardBody className="space-y-3">
            <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: 0.04 }} className="flex items-center justify-between">
              <div className="min-w-0">
                <p className="font-medium">{t("settingscard.body.paths.title", { defaultValue: "内容路径" })}</p>
            ...
            <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: 0.06 }} className="flex items-center justify-between">
              <div>
                <p>{t("settingscard.body.language.name", { defaultValue: t("app.lang") })}</p>
            ...
            <div className="flex items-center justify-end gap-2">
              <Button variant="light" onPress={requestFinish}>{t("onboarding.skip", { defaultValue: "跳过" })}</Button>
              <Button color="primary" onPress={requestFinish}>{t("onboarding.finish", { defaultValue: "完成" })}</Button>
            </div>
          </CardBody>
        </Card>
      </motion.div>
      ...
      <Modal size="md" isOpen={unsavedOpen} onOpenChange={unsavedOnOpenChange} hideCloseButton>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="text-warning-600">{t("onboarding.unsaved.title", { defaultValue: "未保存修改" })}</ModalHeader>
              <ModalBody>
                <div className="text-default-700 text-sm">
                  {t("onboarding.unsaved.body", { defaultValue: "您更改了内容路径但尚未保存。是否保存后完成引导？" })}
                </div>
                {!baseRootWritable && (
                  <div className="text-tiny text-danger-500 mt-1">
                    {t("settingscard.body.paths.not_writable", { defaultValue: "目录不可写入" })}
              ...
              <ModalFooter>
                <Button variant="light" onPress={onClose}>{t("onboarding.unsaved.cancel", { defaultValue: "取消" })}</Button>
                <Button
                  color="primary"
                  isLoading={savingBaseRoot}
                  isDisabled={!newBaseRoot || !baseRootWritable}
